...
void CSDKLoggedInUIMgr::LinstenMeetingServiceEvent()
{
	m_loggedInUIWorkFlow.IsMeetingServiceReady();
}
...
LRESULT CSDKLoggedInUIMgr::OnClose(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	if (SDKInterfaceWrap::GetInst().GetAuthService())
		SDKInterfaceWrap::GetInst().GetAuthService()->LogOut();
...
void CSDKLoggedInUIMgr::onSuspendParticipantsActivities()
{
	//to do
}
...
void CSDKLoggedInUIMgr::DisableMeetingButton()
{
	if(NULL == m_btnJoinMeeting || NULL == m_btnStartAudioOnlyMeeting  || NULL == m_btnStartVideoMeeting
		||NULL == m_btnStartDirectShare || NULL ==  m_btnStartDirectShare)
		return;
