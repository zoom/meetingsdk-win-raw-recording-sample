...
string CSDKSettingApiTest::GetSystemTime()
{
	...
	sprintf_s(szDateTime, 100,"%02d-%02d-%02d %02d:%02d:%02d", m_time.wYear, m_time.wMonth,
...
void CSDKSettingApiTest::Write(std::string &FuncName,ZOOM_SDK_NAMESPACE::SDKError &err)
{
	...
	char Log[MAX_PATH] = {0};	
	sprintf_s(Log, MAX_PATH,"时间：%s----->函数名：%s------>传入参数true ------>返回值: %s",GetSystemTime().c_str(),funcname.c_str(),s_err.c_str());
	string s_Log = Log;
