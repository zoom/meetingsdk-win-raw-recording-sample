...
void CSDKMeetingChatUIMgr::InitWindow()
{
	...
	m_pMsgTypeCombox = static_cast<CComboUI*>(m_PaintManager.FindControl(L"combo_sendchat_msgtype"));
}
...
ZOOM_SDK_NAMESPACE::SDKChatMessageType CSDKMeetingChatUIMgr::ParseChatMsgType()
{
	ZOOM_SDK_NAMESPACE::SDKChatMessageType msgType = ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_None;
	CDuiString sMsgType = m_pMsgTypeCombox->GetText();
	if (sMsgType == _T("None"))
	{
		msgType = ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_None;
	}
	else if (sMsgType == _T("ToAll"))
	{
		msgType = ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_All;
	}
	else if (sMsgType == _T("ToAllPanelist"))
	{
		msgType = ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_All_Panelist;
	}
	else if (sMsgType == _T("ToAllPanelistAndAttendee"))
	{
		msgType = ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_Individual_Panelist;
	}
	else if (sMsgType == _T("ToIndividualUser"))
	{
		msgType = ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_Individual;
	}
	else if (sMsgType == _T("ToWaitingRoom"))
	{
		msgType = ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_WaitingRoomUsers;
	}
...
void CSDKMeetingChatUIMgr::DoPressEnterKey()
{
	...
	ZOOM_SDK_NAMESPACE::SDKChatMessageType msgType(ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_All);
	if (m_pUserList && m_pUserListCombox && m_pMsgTypeCombox)
	{
	...
	if (SendChatMsg(user_id, const_cast<wchar_t*>(chat_info.c_str()), msgType))
	{
		if (ZOOM_SDK_NAMESPACE::SDKChatMessageType_To_All == msgType)
		{
...
bool CSDKMeetingChatUIMgr::SendChatMsg(unsigned int receiver, wchar_t* content, ZOOM_SDK_NAMESPACE::SDKChatMessageType msgType)
{
	return (ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS == m_chat_controller_workflow.SendChatMsg(receiver, content, msgType)) ? true : false;
}
