...
void CUtilWnd::onUserVideoStatusChange( unsigned int userId, ZOOM_SDK_NAMESPACE::VideoStatus status )
{
	...
	if (m_last_video_status != status || m_last_user_id != userId)
	{
		...
		{
			swprintf_s(notify, MAX_PATH,L"user name: %s    user id: %d    video status: VIDEO_ON", user_name.c_str(),userId);
		}
		else
		{
			swprintf_s(notify, MAX_PATH,L"user id: %d    video status: VIDEO_OFF", userId);
		}
