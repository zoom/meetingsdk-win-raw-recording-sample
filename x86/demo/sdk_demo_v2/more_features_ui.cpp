...
void CMoreFeaturesUIMgr::Notify( TNotifyUI& msg )
{
	if (msg.sType == _T("itemclick"))
	{
		...
		else if (msg.pSender == m_PaintManager.FindControl(L"Show AAN Panel"))
		{
			DoClickShowAANPanelBtn();
		}
		else if (msg.pSender == m_PaintManager.FindControl(L"Hide AAN Panel"))
		{
			DoClickHideAANPanelBtn();
		}
	}
}
...
void CMoreFeaturesUIMgr::onUserNamesChanged(ZOOM_SDK_NAMESPACE::IList<unsigned int>* pList)
{
	//todo
	if (m_main_frame)
	{
...
void CMoreFeaturesUIMgr::UpdateUI()
{
	...
	if (pParticipantsCtr)
	{
		bHost = (pParticipantsCtr->GetMySelfUser())->IsHost();
	}
	if (bHost)
	{
	...
	AddElement(L"Show AAN Panel");
	AddElement(L"Hide AAN Panel");
...
void CMoreFeaturesUIMgr::DoClickShowAANPanelBtn()
{
	ZOOM_SDK_NAMESPACE::SDKError err = m_more_features_workflow.ShowAANPanel(1, 1);
	if (ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS != err)
	{
		ShowErrorMsg(CSDKMessInfo::GetInst().GetErrorMessInfo());
	}
}
void CMoreFeaturesUIMgr::DoClickHideAANPanelBtn()
{
	ZOOM_SDK_NAMESPACE::SDKError err = m_more_features_workflow.HideAANPanel();
	if (ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS != err)
	{
		ShowErrorMsg(CSDKMessInfo::GetInst().GetErrorMessInfo());
	}
