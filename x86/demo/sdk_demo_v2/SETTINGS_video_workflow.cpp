...
ZOOM_SDK_NAMESPACE::SDKError CSDKVideoSettingsWorkFlow::EnableTemporalDeNoise(bool bEnable)
{
	if (m_pVideoSettingsContext)
	{
		ZOOM_SDK_NAMESPACE::SDKError err = ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS;
		err = m_pVideoSettingsContext->EnableTemporalDeNoise(bEnable);
		return err;
	}
	return ZOOM_SDK_NAMESPACE::SDKERR_UNINITIALIZE;
}
...
bool CSDKVideoSettingsWorkFlow::IsTemporlDeNoiseEnabled()
{
	if (m_pVideoSettingsContext)
	{
		bool bChecked = m_pVideoSettingsContext->IsTemporalDeNoiseEnabled();
		return bChecked;
	}
	return false;
}
