...
void CSDKCustomUIWorkFlow::InitMeetingService()
{
	...
	if (m_pMeetingConfiguration)
		m_pMeetingConfiguration->SetEvent(this);


	if(m_pCustomUIEvent)
		m_pCustomUIEvent->SetSDKMeetingService(m_pMeetingService);

	if (CustomizedUIRecordMgr::GetInstance())
	{
		CustomizedUIRecordMgr::GetInstance()->GetRecordController();
	}
}
...
bool CSDKCustomUIWorkFlow::IsAudioOn()
{
	...
	do 
	{
		...
		ZOOM_SDK_NAMESPACE::IUserInfo* pMe = m_pUserCtrl->GetMySelfUser();
		if(!pMe)
			break;
...
void CSDKCustomUIWorkFlow::onUserNamesChanged(ZOOM_SDK_NAMESPACE::IList<unsigned int>* pList)
{
	if (m_pCustomUIEvent)
	{
		m_pCustomUIEvent->onUserNamesChanged(pList);
	}
}

void CSDKCustomUIWorkFlow::onWebinarNeedRegisterNotification(ZOOM_SDK_NAMESPACE::IWebinarNeedRegisterHandler* handler_)
{
	if (m_pCustomUIEvent)
	{
		m_pCustomUIEvent->onWebinarNeedRegisterNotification(handler_);
	}
}
