...
public:
	...
	void DoTemporalDeNoiseChkClick();
...
private:
	...
	CCheckBoxUI* m_chkTemporalDeNoise;
	...
	CContainerUI* m_pVieoPreviewLayout = nullptr;
	HWND hwnd;
};
...
{
public:
	CSDKShareSettingsUIGroup();
	virtual ~CSDKShareSettingsUIGroup();
	void InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_);
	void UninitWindow();
	void GetShareSettingsFlags();
	void Show();
	void Hide();
	bool IsVisable()
	{
		...
		{
			return m_settingsSharePage->IsVisible();
		}
		return false;
	}
	void Notify(TNotifyUI& msg);

	void DoEnableTCPConnectionWhenSharingChkClick();
	void DoEnableShowMyAppWindowWhenShare();
	void DoEnableRemoteControlAllApplications();
	void DoEnableGreenBorderWhenShare();
	void DoEnableLimitFPSWhenShare();
	void DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::LimitFPSValue limitValue);
	void DoUseHardwareAccelerationShareChkClick();
	void DoSetWindowSizeTypeWhenShareRadioClick(ZOOM_SDK_NAMESPACE::WindowSizeType eType);
	void DoSetScreenCaptureMode(ZOOM_SDK_NAMESPACE::ScreenCaptureMode eMode);
	void DoEnableHardwareAccelerationVideoShareChkClick();
	void DoEnableHardwareAccelerationAnnotateChkClick();

private:
	CVerticalLayoutUI* m_settingsSharePage;
	CSDKShareSettingsWorkFlow m_shareSettingsWorkFlow;
	CSDKSettingsUIMgr* m_parentFrame;

	CCheckBoxUI* m_chkTCPConnectionWhenSharing = NULL;
	CCheckBoxUI* m_chkShowMyAppWhenShare = NULL;
	CCheckBoxUI* m_chkRemoteControl = NULL;
	CCheckBoxUI* m_chkGreenBorder = NULL;
	CCheckBoxUI* m_chkLimitFrames = NULL;
	CComboUI* m_comboLimitFramesValue = NULL;
	CCheckBoxUI* m_chkUseHardwareAccelerationShare = NULL;
	CCheckBoxUI* m_chkEnableHardwareAccelerationVideoShare = NULL;
	CCheckBoxUI* m_chkEnableHardwareAccelerationAnnotate = NULL;

	COptionUI* m_radioFullScreen = NULL;
	COptionUI* m_radioMaximize = NULL;
	COptionUI* m_radioCurrentSize = NULL;

	CComboUI* m_comboScreenCaptureMode = NULL;
};
...
private:
	...
	CContainerUI* m_pVBVideoPreviewLayout = nullptr;
};
...
public:
	...
	void SetCurrentPage(CVerticalLayoutUI* current_) { UpdateWindow(m_hWnd); m_currentPage = current_; }
...
protected:
	...
	CSDKShareSettingsUIGroup m_ShareSettingsUIGroup;
	...
	CVerticalLayoutUI* m_share_settings_page;
