...
void CCustomizeInMeetingUIMgr::onSize(RECT& rc)
{
	...
	if(m_bInMeeting)
		CToolbarMgr::GetInstance().ResizeToolbarBottom();
...
void CCustomizeInMeetingUIMgr::onDestroyed()
{
	...
	*/
	...
	{
		if(NULL == CToolbarMgr::GetInstance().GetParentWnd())
		{
			CToolbarMgr::GetInstance().SetEvent(m_pAppEvent);
			CToolbarMgr::GetInstance().SetParentWnd(m_hOwnerHwnd);
		}
		CToolbarMgr::GetInstance().EndMeeting();
	}
	else
	{
...
void CCustomizeInMeetingUIMgr::Reset()
{
	...
	m_pJoinWebinarWndUI = nullptr;
}

void CCustomizeInMeetingUIMgr::DestroyAllSubWindow()
{
	CToolbarMgr::GetInstance().UnInit();
	UninitUserList();
	DestroyArrowWindow();
	DestroyChatWindow();
	DestroyParticipantWindow();
...
void CCustomizeInMeetingUIMgr::CreateWebinarWindow(ZOOM_SDK_NAMESPACE::IWebinarNeedRegisterHandler* handler_)
{
	if (NULL == m_pJoinWebinarWndUI)
	{
		m_pJoinWebinarWndUI = new CJoinWebinarWndUI();
		if (m_pJoinWebinarWndUI)
		{
			m_pJoinWebinarWndUI->Create(NULL, _T("Join webnar window"), UI_WNDSTYLE_DIALOG, WS_EX_WINDOWEDGE);
			m_pJoinWebinarWndUI->SetIcon(IDI_ICON_LOGO);
			m_pJoinWebinarWndUI->SetRegisterHandler(handler_);
		}
	}

	if (m_pJoinWebinarWndUI && ::IsWindow(m_pJoinWebinarWndUI->GetHWND()))
	{
		m_pJoinWebinarWndUI->ShowWindow(true);
	}
}

void CCustomizeInMeetingUIMgr::DestroyWebinarWindow()
{
	if (m_pJoinWebinarWndUI)
	{
		if (::IsWindow(m_pJoinWebinarWndUI->GetHWND()))
		{
			::DestroyWindow(m_pJoinWebinarWndUI->GetHWND());
		}
		delete m_pJoinWebinarWndUI;
		m_pJoinWebinarWndUI = NULL;
	}
}
...
void CCustomizeInMeetingUIMgr::CreateToolBar()
{
	...
	CToolbarMgr::GetInstance().SetEvent(m_pAppEvent);
	CToolbarMgr::GetInstance().SetParentWnd(m_hOwnerHwnd);
	CToolbarMgr::GetInstance().HideToolbarBottom();
}
...
void CCustomizeInMeetingUIMgr::ShowToolbar()
{
	...
	CToolbarMgr::GetInstance().ShowToolbarBottom();
}
...
void CCustomizeInMeetingUIMgr::UpdateToolbarButtons()
{
	...
	{
		CToolbarMgr::GetInstance().UpdateButton(Toolbar_Button_Video, Button_UnMuted);
	}
	else
	{
		CToolbarMgr::GetInstance().UpdateButton(Toolbar_Button_Video, Button_Muted);
	}
	if(m_customUIWorkFlow.IsAudioOn())
	{
		CToolbarMgr::GetInstance().UpdateButton(Toolbar_Button_Audio, Button_UnMuted);
	}
	else
	{
		CToolbarMgr::GetInstance().UpdateButton(Toolbar_Button_Audio, Button_Muted);
	}
	if(m_pCustomizeShareMgr)
	{
		if(m_pCustomizeShareMgr->IsInSendShareMode())
			CToolbarMgr::GetInstance().UpdateButton(Toolbar_Button_Share, Button_Muted);
		else
			CToolbarMgr::GetInstance().UpdateButton(Toolbar_Button_Share, Button_UnMuted);
	}
}
...
void CCustomizeInMeetingUIMgr::onUserAudioStatusChange(ZOOM_SDK_NAMESPACE::IList<ZOOM_SDK_NAMESPACE::IUserAudioStatus* >* lstAudioStatusChange, const wchar_t* strAudioStatusList)
{
	...
	ZOOM_SDK_NAMESPACE::IUserInfo* pUserInfo = SDKInterfaceWrap::GetInst().GetMeetingParticipantsController()->GetMySelfUser();
	if (pUserInfo)
	{
...
void CCustomizeInMeetingUIMgr::onWebinarNeedRegisterNotification(ZOOM_SDK_NAMESPACE::IWebinarNeedRegisterHandler* handler_)
{
	if (!handler_)
		return;

	CreateWebinarWindow(handler_);
}

void CCustomizeInMeetingUIMgr::ShowHideGalleryViewWindow()
{
	m_bShowGalleryView = !m_bShowGalleryView;
	CToolbarMgr::GetInstance().SetGalleryViewStatus(m_bShowGalleryView);
...
void CCustomizeInMeetingUIMgr::ShowHideChatWindow()
{
	...
	CToolbarMgr::GetInstance().SetChatUIStatus(m_bInChat);
}
...
void CCustomizeInMeetingUIMgr::UpdateShareButtonStatus()
{
	if(m_pCustomizeShareMgr)
	{
		...
		if(m_pCustomizeShareMgr->IsInSendShareMode())
			bOn =  true;
		CToolbarMgr::GetInstance().SetShareUIStatus(bOn);
	}
	UpdateToolbarButtons();
}
...
void CCustomizeInMeetingUIMgr::ShowHideParticipantWindow()
{
	...
	CToolbarMgr::GetInstance().SetParticipantUIStatus(m_bInShowParticipant);
}
...
void CCustomizeInMeetingUIMgr::UpdateMeetingTitle(bool bInWaitingRoom)
{
	...
	{
		if(bInWaitingRoom)
			swprintf_s(strTitle, title_len,_T("zoom_sdk_demo_ui -- You are in waiting room now"));   
		else if(0 != iMeetingID)
			swprintf_s(strTitle, title_len,_T("zoom_sdk_demo_ui -- meeting ID %I64u"), iMeetingID);   
		else
			swprintf_s(strTitle, title_len,_T("zoom_sdk_demo_ui"));				
...
void CCustomizeInMeetingUIMgr::onUserNamesChanged(ZOOM_SDK_NAMESPACE::IList<unsigned int>* pList)
{
