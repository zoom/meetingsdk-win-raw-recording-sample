...
void AjustDpiScaleRect(RECT& rc)
{
...
void CSDKAudioSettingsUIGroup::InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_)
{
	...
	m_chkMusicMode = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_music_mode")));
...
void CSDKAudioSettingsUIGroup::GetAudioSettingsFlags()
{
	...
	bool bVisiable_ = m_audioSettingsWorkFlow.IsMicOriginalInputEnable();
	bChecked = m_audioSettingsWorkFlow.IsEchoCancellationDisabled();
	if(m_chkEchoCancellation)
	{
		m_chkEchoCancellation->SetVisible(bVisiable_);
		m_chkEchoCancellation->SetCheck(bChecked);
	}
	bChecked = m_audioSettingsWorkFlow.IsHighFidelityMusicModeDisabled();
	if (m_chkMusicMode)
	{
		m_chkMusicMode->SetVisible(bVisiable_);
		m_chkMusicMode->SetCheck(bChecked);
	}
	bChecked = m_audioSettingsWorkFlow.IsStereoAudioEnable();
	if (m_chkStereoAudio)
	{
		m_chkStereoAudio->SetVisible(bVisiable_);
...
void CSDKAudioSettingsUIGroup::Notify(TNotifyUI& msg)
{
	if(msg.sType == _T("click"))
	{
		...
		else if (msg.pSender == m_chkMusicMode)
		{
			DoMusicModeChkClick();
		}
...
void CSDKAudioSettingsUIGroup::DoMicOriginalInputChkClick()
{
	...
	if (bChecked)
	{
		...
		m_chkMusicMode->SetVisible(true);
		m_chkStereoAudio->SetVisible(true);
		bool bChked = m_audioSettingsWorkFlow.IsEchoCancellationDisabled();
		if(m_chkEchoCancellation)
		{
			m_chkEchoCancellation->SetCheck(bChked);
		}
		bChked = m_audioSettingsWorkFlow.IsHighFidelityMusicModeDisabled();
		if (m_chkMusicMode)
		{
			m_chkMusicMode->SetCheck(bChked);
		}
		bChked = m_audioSettingsWorkFlow.IsStereoAudioEnable();
		if (m_chkStereoAudio)
		{
			m_chkStereoAudio->SetCheck(bChked);
		}
	}
	else
	{
		m_chkEchoCancellation->SetVisible(false);
		m_chkMusicMode->SetVisible(false);
		m_chkStereoAudio->SetVisible(false);
	}
...
void CSDKAudioSettingsUIGroup::DoEchoCancellationChkClick()
{
	...
	m_audioSettingsWorkFlow.DisableEchoCancellation(bChecked);
}

void CSDKAudioSettingsUIGroup::DoMusicModeChkClick()
{
	if (NULL == m_chkMusicMode)
		return;
	bool bChecked = !m_chkMusicMode->GetCheck();
	m_audioSettingsWorkFlow.EnableHighFidelityMusicMode(bChecked);
}
...
void CSDKVideoSettingsUIGroup::InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_)
{
	...
	m_chkTemporalDeNoise = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_temporal_denoise")));
	...
	m_pVieoPreviewLayout = static_cast<CContainerUI*>(ui_mgr.FindControl(_T("container_video_preview")));
	m_parentFrame = main_frame_;
...
void CSDKVideoSettingsUIGroup::GetVideoSettingsFlags()
{
	...
	bChecked = m_videoSettingsWorkFlow.IsTemporlDeNoiseEnabled();
	if (m_chkTemporalDeNoise)
	{
		m_chkTemporalDeNoise->SetCheck(bChecked);
	}
}
...
void CSDKVideoSettingsUIGroup::Notify(TNotifyUI& msg)
{
	if(msg.sType == _T("click"))
	{
		...
		else if (msg.pSender == m_chkTemporalDeNoise)
		{
			DoTemporalDeNoiseChkClick();
		}
...
void CSDKVideoSettingsUIGroup::DoTemporalDeNoiseChkClick()
{
	if (NULL == m_chkTemporalDeNoise)
		return;
	bool bChecked = !m_chkTemporalDeNoise->GetCheck();
	m_videoSettingsWorkFlow.EnableTemporalDeNoise(bChecked);
}
...
{
	RECT rect = { 0,0,0,0 };
	if(m_pVieoPreviewLayout)
		rect = m_pVieoPreviewLayout->GetPos();

	AjustDpiScaleRect(rect);

	m_videoSettingsWorkFlow.SetVideoPreviewParentWnd(hwnd, rect);
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///class CSDKShareSettingsUIGroup
CSDKShareSettingsUIGroup::CSDKShareSettingsUIGroup()
{
	m_chkTCPConnectionWhenSharing = NULL;
	m_chkRemoteControl = NULL;
	m_chkGreenBorder = NULL;
	m_chkUseHardwareAccelerationShare = NULL;
	m_chkEnableHardwareAccelerationVideoShare = NULL;
	m_chkEnableHardwareAccelerationAnnotate = NULL;

	m_radioFullScreen = NULL;
	m_radioMaximize = NULL;
	m_radioCurrentSize = NULL;

	m_comboScreenCaptureMode = NULL;
}
CSDKShareSettingsUIGroup::~CSDKShareSettingsUIGroup()
{
	m_chkTCPConnectionWhenSharing = NULL;
...
void CSDKShareSettingsUIGroup::InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_)
{
	m_settingsSharePage = static_cast<CVerticalLayoutUI*>(ui_mgr.FindControl(_T("panel_Share_Settings")));
	m_chkTCPConnectionWhenSharing = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_tcp_connection_when_sharing")));
	m_chkShowMyAppWhenShare = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_enable_show_my_app_when_share")));
	m_chkRemoteControl = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_romote_control")));
	m_chkGreenBorder = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_green_border")));
	m_comboLimitFramesValue = static_cast<CComboUI*>(ui_mgr.FindControl(_T("combo_LimitPSVValue")));
	m_chkLimitFrames = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_limit_frames")));
	m_chkUseHardwareAccelerationShare = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_use_hardware_screen_sharing")));
	m_chkEnableHardwareAccelerationVideoShare = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_enable_hardware_screen_video_sharing")));
	m_chkEnableHardwareAccelerationAnnotate = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_enable_hardware_screen_annotate")));

	m_radioFullScreen = static_cast<COptionUI*>(ui_mgr.FindControl(_T("radio_full_screen")));
	m_radioMaximize = static_cast<COptionUI*>(ui_mgr.FindControl(_T("radio_maximize_win")));
	m_radioCurrentSize = static_cast<COptionUI*>(ui_mgr.FindControl(_T("radio_cur_size")));

	m_comboScreenCaptureMode = static_cast<CComboUI*>(ui_mgr.FindControl(_T("combo_screen_capture_mode")));

	m_parentFrame = main_frame_;
	GetShareSettingsFlags();
}
void CSDKShareSettingsUIGroup::UninitWindow()
{
	m_shareSettingsWorkFlow.Cleanup();
}
void CSDKShareSettingsUIGroup::GetShareSettingsFlags()
{
	...
	bChecked = m_shareSettingsWorkFlow.IsRemoteControlAllApplicationsEnabled();
	if (m_chkRemoteControl)
	{
		m_chkRemoteControl->SetCheck(bChecked);
	}
	bChecked = m_shareSettingsWorkFlow.IsGreenBorderEnabledWhenShare();
	if (m_chkGreenBorder)
	{
		m_chkGreenBorder->SetCheck(bChecked);
	}
	bChecked = m_shareSettingsWorkFlow.IsLimitFPSEnabledWhenShare();
	if (m_chkLimitFrames)
	{
		m_chkLimitFrames->SetCheck(bChecked);
		m_comboLimitFramesValue->SetEnabled(bChecked);
	}
	bIsSupport = m_shareSettingsWorkFlow.IsCurrentOSSupportAccelerateGPUWhenShare();
	bChecked = m_shareSettingsWorkFlow.IsAccelerateGPUWhenShareEnabled();
	if (m_chkUseHardwareAccelerationShare)
	{
		if (!bIsSupport)
		{
			m_chkUseHardwareAccelerationShare->SetVisible(false);
		}
		else
		{
			m_chkUseHardwareAccelerationShare->SetVisible(true);
			m_chkUseHardwareAccelerationShare->SetCheck(bChecked);
		}	
	}
	bChecked = m_shareSettingsWorkFlow.IsAnnotateHardwareAccelerationEnabled();
	if (m_chkEnableHardwareAccelerationAnnotate)
	{
		if (!bIsSupport)
		{
			m_chkEnableHardwareAccelerationAnnotate->SetVisible(false);
		}
		else
		{
			m_chkEnableHardwareAccelerationAnnotate->SetVisible(true);
			m_chkEnableHardwareAccelerationAnnotate->SetCheck(bChecked);
		}
	}
	bChecked = m_shareSettingsWorkFlow.IsAccelerateGPUWhenVideoShareEnabled();
	if (m_chkEnableHardwareAccelerationVideoShare)
	{
		if (!bIsSupport)
		{
			m_chkEnableHardwareAccelerationVideoShare->SetVisible(false);
		}
		else
		{
			m_chkEnableHardwareAccelerationVideoShare->SetVisible(true);
			m_chkEnableHardwareAccelerationVideoShare->SetCheck(bChecked);
		}
	}

	ZOOM_SDK_NAMESPACE::WindowSizeType eWindowSizeType = m_shareSettingsWorkFlow.GetWindowSizeTypeWhenViewShare();
	switch (eWindowSizeType)
	{
	case ZOOMSDK::WindowSize_FullScreen:
	{
		if (m_radioFullScreen)
		{
			m_radioFullScreen->Selected(true, false);
		}
	}
	break;
	case ZOOMSDK::WindowSize_Maximize:
	{
		if (m_radioMaximize)
		{
			m_radioMaximize->Selected(true, false);
		}
	}
	break;
	case ZOOMSDK::WindowSize_CurrentSize:
	{
		if (m_radioCurrentSize)
		{
			m_radioCurrentSize->Selected(true, false);
		}
	}
	break;
	default:
	...
	ZOOM_SDK_NAMESPACE::ScreenCaptureMode eMode;
	m_shareSettingsWorkFlow.GetScreenCaptureMode(eMode);
	m_comboScreenCaptureMode->SetInternVisible(true);
	switch (eMode)
	{
	case ZOOMSDK::CaptureMode_auto:
		m_comboScreenCaptureMode->SelectItem(0, false, false);
		break;
	case ZOOMSDK::CaptureMode_legacy:
		m_comboScreenCaptureMode->SelectItem(4, false, false);
		break;
	case ZOOMSDK::CaptureMode_gpu_copy_filter:
		m_comboScreenCaptureMode->SelectItem(1, false, false);
		break;
	case ZOOMSDK::CaptureMode_ada_copy_filter:
		m_comboScreenCaptureMode->SelectItem(2, false, false);
		break;
	case ZOOMSDK::CaptureMode_ada_copy_without_filter:
		m_comboScreenCaptureMode->SelectItem(3, false, false);
		break;
	default:
		break;
	}

	if (m_comboLimitFramesValue)
	{
		ZOOM_SDK_NAMESPACE::LimitFPSValue value = m_shareSettingsWorkFlow.GetLimitFPSValueWhenShare();
		m_comboLimitFramesValue->SetInternVisible(true);
		switch (value)
		{
		case ZOOMSDK::limitfps_Not_Enable:
			m_comboLimitFramesValue->SetText(_T(""));
			break;
		case ZOOMSDK::limitfps_1_frame:
			m_comboLimitFramesValue->SelectItem(6);
			break;
		case ZOOMSDK::limitfps_2_frame:
			m_comboLimitFramesValue->SelectItem(5);
			break;
		case ZOOMSDK::limitfps_4_frame:
			m_comboLimitFramesValue->SelectItem(4);
			break;
		case ZOOMSDK::limitfps_6_frame:
			m_comboLimitFramesValue->SelectItem(3);
			break;
		case ZOOMSDK::limitfps_8_frame:
			m_comboLimitFramesValue->SelectItem(2);
			break;
		case ZOOMSDK::limitfps_10_frame:
			m_comboLimitFramesValue->SelectItem(1);
			break;
		case ZOOMSDK::limitfps_15_frame:
			m_comboLimitFramesValue->SelectItem(0);
			break;
		default:
			break;
		}
...
void CSDKShareSettingsUIGroup::Show()
{
	if (m_settingsSharePage)
	{
		m_settingsSharePage->SetVisible(true);
		if (m_parentFrame)
		{
			m_parentFrame->SetCurrentPage(m_settingsSharePage);
		}
	}
}
void CSDKShareSettingsUIGroup::Hide()
{
	if (m_settingsSharePage)
	{
		m_settingsSharePage->SetVisible(false);
	}
}
void CSDKShareSettingsUIGroup::Notify(TNotifyUI& msg)
{
	if (msg.sType == _T("click"))
	{
		if (msg.pSender == m_chkTCPConnectionWhenSharing)
		{
			DoEnableTCPConnectionWhenSharingChkClick();
		}
		else if (msg.pSender == m_chkShowMyAppWhenShare)
		{
			DoEnableShowMyAppWindowWhenShare();
		}
		else if (msg.pSender == m_chkRemoteControl)
		{
			DoEnableRemoteControlAllApplications();
		}
		else if (msg.pSender == m_chkGreenBorder)
		{
			DoEnableGreenBorderWhenShare();
		}
		else if (msg.pSender == m_chkLimitFrames)
		{
			DoEnableLimitFPSWhenShare();
		}
		else if (msg.pSender == m_chkUseHardwareAccelerationShare)
		{
			DoUseHardwareAccelerationShareChkClick();
		}
		else if (msg.pSender == m_chkEnableHardwareAccelerationAnnotate)
		{
			DoEnableHardwareAccelerationAnnotateChkClick();
		}
		else if (msg.pSender == m_chkEnableHardwareAccelerationVideoShare)
		{
			DoEnableHardwareAccelerationVideoShareChkClick();
		}
		else if (msg.pSender == m_radioFullScreen)
		{
			DoSetWindowSizeTypeWhenShareRadioClick(ZOOM_SDK_NAMESPACE::WindowSize_FullScreen);
		}
		else if (msg.pSender == m_radioMaximize)
		{
			DoSetWindowSizeTypeWhenShareRadioClick(ZOOM_SDK_NAMESPACE::WindowSize_Maximize);
		}
		else if (msg.pSender == m_radioCurrentSize)
		{
			DoSetWindowSizeTypeWhenShareRadioClick(ZOOM_SDK_NAMESPACE::WindowSize_CurrentSize);
		}
	}
	else if (msg.sType == _T("itemselect"))
	{
		if (msg.pSender == m_comboScreenCaptureMode)
		{
			int iSelIndex = m_comboScreenCaptureMode->GetCurSel();

			if (iSelIndex == 0)
			{
				DoSetScreenCaptureMode(ZOOM_SDK_NAMESPACE::CaptureMode_auto);
			}
			else if (iSelIndex == 1)
			{
				DoSetScreenCaptureMode(ZOOM_SDK_NAMESPACE::CaptureMode_gpu_copy_filter);
			}
			else if (iSelIndex == 2)
			{
				DoSetScreenCaptureMode(ZOOM_SDK_NAMESPACE::CaptureMode_ada_copy_filter);
			}
			else if (iSelIndex == 3)
			{
				DoSetScreenCaptureMode(ZOOM_SDK_NAMESPACE::CaptureMode_ada_copy_without_filter);
			}
			else if (iSelIndex == 4)
			{
				DoSetScreenCaptureMode(ZOOM_SDK_NAMESPACE::CaptureMode_legacy);
			}
		}
		else if (msg.pSender == m_comboLimitFramesValue)
		{
			int iSelIndex = m_comboLimitFramesValue->GetCurSel();

			if (iSelIndex == 0)
			{
				DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::limitfps_15_frame);
			}
			else if (iSelIndex == 1)
			{
				DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::limitfps_10_frame);
			}
			else if (iSelIndex == 2)
			{
				DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::limitfps_8_frame);
			}
			else if (iSelIndex == 3)
			{
				DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::limitfps_6_frame);
			}
			else if (iSelIndex == 4)
			{
				DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::limitfps_4_frame);
			}
			else if (iSelIndex == 5)
			{
				DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::limitfps_2_frame);
			}
			else if (iSelIndex == 6)
			{
				DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::limitfps_1_frame);
			}
		}
	}
}
void CSDKShareSettingsUIGroup::DoEnableTCPConnectionWhenSharingChkClick()
{
	if (NULL == m_chkTCPConnectionWhenSharing)
		return;
	bool bChecked = !m_chkTCPConnectionWhenSharing->GetCheck();
	m_shareSettingsWorkFlow.EnableTCPConnectionWhenSharing(bChecked);
}
void CSDKShareSettingsUIGroup::DoEnableShowMyAppWindowWhenShare()
{
	if (NULL == m_chkShowMyAppWhenShare)
		return;
	bool bChecked = !m_chkShowMyAppWhenShare->GetCheck();
	m_shareSettingsWorkFlow.EnableShowMyAppWindowWhenShare(bChecked);
}
void CSDKShareSettingsUIGroup::DoEnableRemoteControlAllApplications()
{
	if (NULL == m_chkRemoteControl)
		return;
	bool bChecked = !m_chkRemoteControl->GetCheck();
	m_shareSettingsWorkFlow.EnableRemoteControlAllApplications(bChecked);
}
void CSDKShareSettingsUIGroup::DoEnableGreenBorderWhenShare()
{
	if (NULL == m_chkGreenBorder)
		return;
	bool bChecked = !m_chkGreenBorder->GetCheck();
	m_shareSettingsWorkFlow.EnableGreenBorderWhenShare(bChecked);
}
void CSDKShareSettingsUIGroup::DoEnableLimitFPSWhenShare()
{
	if (NULL == m_chkLimitFrames)
		return;
	bool bChecked = !m_chkLimitFrames->GetCheck();
	...
	m_shareSettingsWorkFlow.EnableLimitFPSWhenShare(bChecked);
}
void CSDKShareSettingsUIGroup::DoSetLimitFPSValueWhenShare(ZOOM_SDK_NAMESPACE::LimitFPSValue limitValue)
{
	m_shareSettingsWorkFlow.SetLimitFPSValueWhenShare(limitValue);
}
void CSDKShareSettingsUIGroup::DoUseHardwareAccelerationShareChkClick()
{
	if (NULL == m_chkUseHardwareAccelerationShare)
		return;
	bool bChecked = !m_chkUseHardwareAccelerationShare->GetCheck();
	m_shareSettingsWorkFlow.EnableAccelerateGPUWhenShare(bChecked);
}
void CSDKShareSettingsUIGroup::DoSetWindowSizeTypeWhenShareRadioClick(ZOOM_SDK_NAMESPACE::WindowSizeType eType)
{
	m_shareSettingsWorkFlow.SetWindowSizeTypeWhenViewShare(eType);
}
void CSDKShareSettingsUIGroup::DoSetScreenCaptureMode(ZOOM_SDK_NAMESPACE::ScreenCaptureMode eMode)
{
	if (NULL == m_comboScreenCaptureMode)
		return;
	m_shareSettingsWorkFlow.SetScreenCaptureMode(eMode);
}

void CSDKShareSettingsUIGroup::DoEnableHardwareAccelerationVideoShareChkClick()
{
	if (NULL == m_chkEnableHardwareAccelerationVideoShare)
		return;
	bool bChecked = !m_chkEnableHardwareAccelerationVideoShare->GetCheck();
	m_shareSettingsWorkFlow.EnableAccelerateGPUWhenVideoShare(bChecked);
}

void CSDKShareSettingsUIGroup::DoEnableHardwareAccelerationAnnotateChkClick()
{
	if (NULL == m_chkEnableHardwareAccelerationAnnotate)
		return;
	bool bChecked = !m_chkEnableHardwareAccelerationAnnotate->GetCheck();
	m_shareSettingsWorkFlow.EnableAnnotateHardwareAcceleration(bChecked);
}
...
void CSDKVBGImageList::ClearAll()
{
	if(!m_pImageList.empty())
		m_pImageList.clear();
	m_mapIndexType.erase(m_mapIndexType.begin(), m_mapIndexType.end());
}
...
void CSDKVirtualBGSettingsUIGroup::InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_)
{
	...
	m_pVBVideoPreviewLayout = static_cast<CContainerUI*>(ui_mgr.FindControl(_T("container_vb_video_preview")));
...
{
	RECT rect = { 0,0,0,0 };
	if(m_pVBVideoPreviewLayout)
		rect = m_pVBVideoPreviewLayout->GetPos();

	AjustDpiScaleRect(rect);

	m_vbgSettingsWorkFlow.SetVideoPreviewParentWnd(m_hPreviewWnd, rect);
}
...
void CSDKVirtualBGSettingsUIGroup::DoRemoveImageBtnClick()
{
	if(m_pVBGImageList && m_pImageListUI)
	{
		...
		if(pImageInfo)
		{
			if (ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS == m_vbgSettingsWorkFlow.RemoveBGImage(pImageInfo, itemType))
				UpdateListUI();
		}
	}
}
...
void CSDKSettingsUIMgr::InitWindow()
{
	...
	m_ShareSettingsUIGroup.InitWindow(m_PaintManager, this);
	...
	m_btnShareSettings = static_cast<COptionUI*>(m_PaintManager.FindControl(_T("tabbtn_share_settings")));
	m_share_settings_page = static_cast<CVerticalLayoutUI*>(m_PaintManager.FindControl(_T("panel_Share_Settings")));
...
void CSDKSettingsUIMgr::Notify( TNotifyUI& msg )
{
	if(msg.sType == _T("click"))
	{
		...
		else if (msg.pSender == m_btnShareSettings && m_currentPage != m_share_settings_page)
		{
			SwitchToPage(Setting_Share_Page);
		}
		...
		else if (m_currentPage == m_share_settings_page)
		{
			m_ShareSettingsUIGroup.Notify(msg);
		}
...
void CSDKSettingsUIMgr::SwitchToPage(SettingsUI_page nPage)
{
	...
	if (m_ShareSettingsUIGroup.IsVisable())
		m_ShareSettingsUIGroup.Hide();
	switch(nPage)
	{
	...
	case Setting_Video_Page:
		...
		break;
	case Setting_Share_Page:
		m_ShareSettingsUIGroup.Show();
		break;
