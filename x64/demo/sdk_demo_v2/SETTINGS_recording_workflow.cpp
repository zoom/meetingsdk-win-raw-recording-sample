...
ZOOM_SDK_NAMESPACE::SDKError CSDKRecordingSettingsWorkFlow::CanGetRecordingManagementURL(bool& bEnable)
{
	if(m_pRecordingSettingContext)
	{
		ZOOM_SDK_NAMESPACE::SDKError err = ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS;
		err = m_pRecordingSettingContext->CanGetRecordingManagementURL(bEnable);
		if (bEnable)
		{
			err = m_pRecordingSettingContext->GetCloudRecordingStorageInfo();
		}
		return err;
	}
	return ZOOM_SDK_NAMESPACE::SDKERR_UNINITIALIZE;
