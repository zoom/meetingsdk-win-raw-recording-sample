...
{
	return SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingAudioController()->EnableMuteOnEntry(bEnable, true);
}

ZOOM_SDK_NAMESPACE::SDKError CMoreFeaturesWorkflow::LowerAllHands()
{
	return SDKInterfaceWrap::GetInst().GetMeetingParticipantsController()->LowerAllHands(false);
}
...
ZOOM_SDK_NAMESPACE::SDKError CMoreFeaturesWorkflow::ShowAANPanel(unsigned int x, unsigned int y)
{
	return SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingAANController()->ShowAANPanel(x, y);
}
ZOOM_SDK_NAMESPACE::SDKError CMoreFeaturesWorkflow::HideAANPanel()
{
	return SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingAANController()->HideAANPanel();
}